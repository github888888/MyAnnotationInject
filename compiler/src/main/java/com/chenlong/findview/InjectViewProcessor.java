 ...
 */
@SupportedAnnotationTypes({"com.chenlong.findview.InjectView", "com.chenlong.findlayout.InjectLayout"})
...
public class InjectViewProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for(Element element : elementsAnnotatedWith) {
            ...
            MemberInfo meminfo = new MemberInfo(id, name, type);
            ...
            if(map.containsKey(qualifiedName)) {
                ...
                // 获取布局资源id
                InjectLayout annotation = enclosingElement.getAnnotation(InjectLayout.class);
                if(null == annotation) {
                ...
                for (Element item : enclosedElements) {
                    if(null == item.getAnnotation(ClickView.class)) continue;
                    clickids = item.getAnnotation(ClickView.class).value();
                    break;
                }
    ...
    private void generateFileByClazzInfo(ClazzInfo clazzInfo) {
        try {
            ...
            for (Map.Entry<Integer, MemberInfo> item : clazzInfo.memberMap.entrySet()) {
            ...
            if(null == clazzInfo.clickIds || clazzInfo.clickIds.length == 0) {
                ...
                for(int item : clazzInfo.clickIds) {
                    MemberInfo info = clazzInfo.memberMap.get(item);
